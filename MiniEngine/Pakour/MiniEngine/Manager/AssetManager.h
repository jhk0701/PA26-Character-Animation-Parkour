...
namespace MiniEngine
{
    ...
    public:
        bool IsValid(const std::wstring& _path) const;

        // .mini StaticMesh 로드(+ GPU 리소스 생성). 이미 살아있으면 캐시된 핸들 반환.
        // 실패 시 nullptr.
        std::shared_ptr<StaticMesh> LoadStaticMesh(const std::wstring& _path, ID3D11Device* _device);

        // .mini SkinnedMesh(통합 컨테이너) 로드(+ GPU 리소스 생성). 캐시 규칙 동일.
        std::shared_ptr<SkinnedMesh> LoadSkinnedMesh(const std::wstring& _path, ID3D11Device* _device);
