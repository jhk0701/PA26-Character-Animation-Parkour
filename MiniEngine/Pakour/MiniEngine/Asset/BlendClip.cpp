...
namespace MiniEngine
{
	...
	void BlendClip::Sample(float _dt, Skeleton& _skeleton, LocalPoseTRS& _outPose)
	{
		...
		for (const Placement& p : m_placements)
			durSec += p.m_pClip->ClipDurationSec() * p.m_weight;
		if (durSec < kEps)
			durSec = kEps;

		m_blendPhase += _dt / durSec;
		m_blendPhase -= std::floor(m_blendPhase);
		...
		for (const Placement& p : m_placements)
		{
			...
			const float timeSec = m_blendPhase * p.m_pClip->ClipDurationSec();
			p.m_pClip->SampleTRS(timeSec, _skeleton, m_poseScratch); // 임시 포즈에 현재 클립 원본 자세 보관
			...
			else
			{
				...
				p.m_pClip->SampleTRS(timeSec, _skeleton, m_poseScratch);
